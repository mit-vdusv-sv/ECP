...
## Agregar como aplicación web

El paquete incluye `manifest.webmanifest`, iconos PNG de 192 y 512 píxeles, una variante adaptable al recorte del dispositivo y `apple-touch-icon` para la pantalla de inicio de iPhone/iPad. Conservan el diseño del icono ECP y están enlazados desde las tres páginas.

Desde el sitio publicado, usa la opción de instalar la aplicación o añadirla a la pantalla de inicio de tu navegador. La aplicación se llama **ECP** y abre la portada con acceso a las dos herramientas. Su disponibilidad depende del navegador y del dispositivo. Esta configuración sigue las referencias de [MDN](https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Guides/Making_PWAs_installable) y [WebKit](https://webkit.org/blog/13878/web-push-for-web-apps-on-ios-and-ipados/).

No se incluye almacenamiento de páginas sin conexión; la instalación requiere acceder al sitio. Los registros conservan el mecanismo de almacenamiento local descrito abajo.
...
3. Usa **Buscar palabras clave**, visible sobre el recorrido, para localizar términos en nombres, explicaciones, contexto y texto original. Por ejemplo, `permeable` encuentra «Pavimentos permeables», «SUDS» y las páginas 24, 25, 47 y 104. Selecciona **Ver concepto en la red** para abrir su ubicación y explicación con el término resaltado, o una coincidencia documental para leer su página. **Volver a resultados** conserva la consulta. La búsqueda ignora mayúsculas y tildes y admite varias palabras; no añade definiciones ajenas a la Guía. Los enlaces conservan el concepto seleccionado y funcionan con Atrás y recarga.
...
manifest.webmanifest               Nombre, iconos y acceso de la aplicación instalada
assets/icons/                      Iconos PNG de instalación y pantalla de inicio
...
```sh
node --test tests/model.test.mjs tests/guide.test.mjs tests/review.test.mjs tests/search.test.mjs
```
