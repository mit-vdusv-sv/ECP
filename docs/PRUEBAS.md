...
## Icono de aplicación web · v2.0.2

Se añadieron el manifiesto compartido y cuatro iconos PNG derivados del SVG existente: 192 px, 512 px, adaptable de 512 px y Apple de 180 px. Comprobación en Chrome con un perfil de pruebas independiente: las tres páginas, tanto en la raíz como bajo `/repositorio/`, resuelven el manifiesto y decodifican los cuatro iconos en sus dimensiones declaradas. El navegador no reporta errores de manifiesto ni de instalabilidad en estas seis rutas. Se revisó visualmente el PNG de 512 px. No se realizó una instalación física en iPhone/Android ni se añadió soporte sin conexión.


## Búsqueda por palabras clave · v2.1.0

Cinco pruebas de búsqueda y cuatro de integridad documental aprobadas. Se verificaron coincidencias parciales y múltiples, prioridad de nombres, tildes/mayúsculas, contexto, páginas originales, consultas vacías y resaltado seguro de texto. El ejemplo «permeable» devuelve dos conceptos y cuatro páginas sin atribuir todo el contenido de una página a cada concepto que la referencia.

Siete comprobaciones de interfaz aprobadas en Chrome, Edge, Firefox y WebKit: navegación desde resultados y regreso, lector con resaltado, consultas sin coincidencias y HTML escapado, teclado, móvil de 360 px y texto al 200 %, respuestas tardías que no interrumpen la navegación, reintento si falla la lectura del documento y ausencia de errores JavaScript o cambios en el almacenamiento del piloto. Revisión visual de capturas de escritorio y móvil. Los perfiles son de pruebas; WebKit no equivale a una prueba en un dispositivo Apple físico. Resultados en `pruebas-buscador.json`.

```sh
node --test tests/search.test.mjs tests/guide.test.mjs
node tests/search-browser.cjs
```

La prueba de navegador utiliza Playwright, `QA_ENGINE`, `TEST_BASE_URL` y opcionalmente `TEST_OUTPUT_DIR`, como las anteriores.


## Ajuste del lector al ancho de pantalla · v2.1.1

El lector deja de forzar los saltos de línea extraídos de las columnas del PDF. Las líneas se ajustan al ancho disponible y se agrega espacio entre bloques cuando una línea del original termina en punto, interrogación o exclamación. Esta separación es de presentación, no una reconstrucción de la diagramación del PDF.

Comprobación en Chrome a 1440 y 390 píxeles: página 47 sin desbordamiento horizontal, líneas que utilizan el ancho disponible y texto idéntico al original extraído. Se verificaron el resaltado de «permeable», la navegación a la página 104 y el regreso a la 47. Capturas de escritorio y móvil revisadas. Se actualizaron las versiones de los recursos para evitar que la caché conserve el formato anterior.
